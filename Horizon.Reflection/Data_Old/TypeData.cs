...
namespace Horizon.Reflection
{
    ...
    public sealed class TypeData : ModifierData
    {
        ...
        internal TypeData(Type type) : base(type.GetModifierFlags(), type.Name, type.FullName)
        {
            ...
            _isNullable = new Lazy<bool>(() => !(this | DefinitionFlags.Value) || System.Nullable.GetUnderlyingType(_type) != null);
        }
        ...
        {
            return typeData == this || Extends(typeData, false) || Implements(typeData, false);
        }
        ...
        {
            return Extends(baseTypeData, true);
        }
        ...
        {
            return Implements(interfaceTypeData, true);
        }
        ...
        public bool TryGetGenericTypeDefinition(out TypeData genericTypeDefinition)
        {
            if (!(this & DefinitionFlags.ConstructedGeneric))
            {
            ...
            try
            {
                genericTypeDefinition = _type.GetGenericTypeDefinition().GetTypeData();
                return true;
            }
            catch (Exception)
            {
        ...
        /// </summary>
        /// <param name="baseTypeData">Base type data.</param>
        /// <param name="genericDeconstruction">If possible, compare against generic type definitions?</param>
        /// <returns>True if the current <see cref="TypeData"/> extends the specified <see cref="TypeData"/>; otherwise, false.</returns>
        private bool Extends(TypeData baseTypeData, bool genericDeconstruction)
        {
            ...
            var deconstructedGeneric = genericDeconstruction && baseTypeData | DefinitionFlags.DeconstructedGeneric;

            var typeData = BaseType;

            while (typeData != null)
            {
                if (deconstructedGeneric ? typeData.TryGetGenericTypeDefinition(out var genericTypeDefinition) && baseTypeData == genericTypeDefinition : typeData == baseTypeData)
                {
                    return true;
                }
        ...
        /// <param name="genericDeconstruction">If possible, compare against generic type definitions?</param>
        /// <returns>True if the current <see cref="TypeData"/> implements the specified <see cref="TypeData"/>; otherwise, false.</returns>
        private bool Implements(TypeData interfaceTypeData, bool genericDeconstruction)
        {
            ...
            var deconstructedGeneric = genericDeconstruction && interfaceTypeData | DefinitionFlags.DeconstructedGeneric;

            return Interfaces.Any(typeData => deconstructedGeneric
                ? typeData.TryGetGenericTypeDefinition(out var genericTypeDefinition) &&
                  interfaceTypeData == genericTypeDefinition
                : interfaceTypeData == typeData);
        }
    }
}